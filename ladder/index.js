...
const getEligibleProblems = async (user) => {
  ...
  allSubmissions = allSubmissions.filter(
    (s) => s.creationTimeSeconds >= thresholdTime && s.verdict === "OK"
  );
  ...
  for (let start = thresholdTime; start < now; start += observationWindow) {
    ...
    const ratingWindow = rating.filter(
      (r) =>
        r.ratingUpdateTimeSeconds >= start && r.ratingUpdateTimeSeconds < end
    );
    ...
    if (maxRating - startRating >= improvementThreshold) {
      eligibleSubmissions = [
        ...eligibleSubmissions,
        ...submissions.filter(
          (s) => s.creationTimeSeconds >= start && s.creationTimeSeconds < end
        ),
      ];
    }
  }
...
const getEligibleProblemsInBatch = async (
  users,
  problemMap,
  problemFreq,
  errCount,
...
const getAllEligibleProblems = async (startFrom, doSkipped = false) => {
  ...
  for (let i = start; i < users.length; i += checkPointSize) {
    ...
    fs.writeFileSync(
      `./checkpoints/checkpoint_${checkpoint}.json`,
      JSON.stringify(toWrite)
    );
    console.log(
      `Checkpoint upto ${checkpoint} written, skipped users: ${
        skippedUsers.length
      }, problems: ${Object.keys(problemMap).length}`
    );

    let tasks = [];
    for (let j = 0; j < checkPointSize; j += batchSize) {
      tasks.push(
        getEligibleProblemsInBatch(
          users,
          problemMap,
          problemFreq,
          errCount,
  ...
  fs.writeFileSync(
    `./checkpoints/checkpoint_${users.length}.json`,
    JSON.stringify(toWrite)
  );
  console.log("Done");
};
